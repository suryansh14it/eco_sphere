...
export default function QuizModal({
  ...
  // Handle marks and animation phases
  useEffect(() => {
    if (currentStep === 'results') {
      // First show marks, then trigger animation after a delay
      const marksTimer = setTimeout(() => {
        setShowMarks(false);
        setShowAnimation(true);
      }, 2500); // Show marks for 2.5 seconds
      
      return () => clearTimeout(marksTimer);
    }
  }, [currentStep]);
  ...
  const renderQuizContent = () => {
    ...
    if (currentStep === 'results' && quizResults) {
      ...
      const scoreBackgroundClass = quizResults.score >= 70 
      ...
      return (
        <div className="space-y-8">
          {showMarks && (
            <div className={`text-center p-8 border-2 rounded-xl ${scoreBackgroundClass} relative overflow-hidden mb-6`}>
              <div className="flex flex-col items-center justify-center gap-4">
                <div className="text-6xl font-bold mb-2 flex items-center">
                  <span className={scoreClassName}>{quizResults.score}</span>
                  <span className="text-3xl text-gray-400 ml-1">%</span>
                </div>
                
                <div className="text-2xl font-medium">
                  <span className="text-muted-foreground">You got </span>
                  <span className={`${scoreClassName} font-bold`}>{quizResults.correctAnswers}</span>
                  <span className="text-muted-foreground"> out of </span>
                  <span className={`${scoreClassName} font-bold`}>{quizResults.totalQuestions}</span>
